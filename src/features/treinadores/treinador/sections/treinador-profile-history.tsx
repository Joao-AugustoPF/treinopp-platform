...
import type { ITreinadorHistorico } from '../types';
...
export function TreinadorProfileHistory({
  createdAt,
  createdBy,
  updatedAt,
  updatedBy,
        ...
        {historico.map((item, index) => (
          <Stack key={item.Id} spacing={1}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              />
              <Box>
                <Box sx={{ typography: 'subtitle2' }}>Academia: {item.AcademiaId}</Box>
                <Box sx={{ typography: 'caption', color: 'text.secondary' }}>
                  Período: {fDateTime(item.DataInicio)} -{' '}
                  {item.DataFim ? fDateTime(item.DataFim) : 'Presente'}
                </Box>
              </Box>
            ...
            <Box sx={{ typography: 'body2' }}>{item.Motivo}</Box>
            {item.Observacao && (
              <Box sx={{ typography: 'body2', color: 'text.secondary' }}>{item.Observacao}</Box>
            )}
